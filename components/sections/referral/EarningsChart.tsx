  ...
  if (isLoading) {
    return (
      <section className="px-2 sm:px-4 mb-4 sm:mb-6">
        <h3 className="text-sm sm:text-lg font-orbitron font-bold mb-3 sm:mb-4">Earnings by Level</h3>
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-700">
          <div className="text-center text-gray-400">
            ...
            <p className="text-sm sm:text-base">Loading earnings data...</p>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section className="px-2 sm:px-4 mb-4 sm:mb-6">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-gray-700">
        <div className="h-40 sm:h-56 md:h-64 flex items-end justify-around space-x-1 sm:space-x-2">
          {levels.map((level, index) => (
            <div key={level.level} className="flex-1 flex flex-col items-center space-y-1 sm:space-y-2">
              <div className="text-xs text-gray-400 hidden sm:block">${level.amount}</div>
              <div
                className="w-full bg-gradient-to-t from-neon-blue via-electric-purple to-neon-blue rounded-t-lg transition-all hover:opacity-80 touch-manipulation"
                style={{ 
        ...
        <div className="mt-3 sm:mt-4 grid grid-cols-3 gap-2 sm:gap-4">
          <div className="text-center">
            <p className="text-sm sm:text-lg font-bold text-neon-blue">${level1to3.toFixed(0)}</p>
            <p className="text-xs text-gray-400">Level 1-3</p>
          </div>
          <div className="text-center">
            <p className="text-sm sm:text-lg font-bold text-electric-purple">${level4to6.toFixed(0)}</p>
            <p className="text-xs text-gray-400">Level 4-6</p>
          </div>
          <div className="text-center">
            <p className="text-sm sm:text-lg font-bold text-green-400">${level7to10.toFixed(0)}</p>
